...
.. versionchanged:: 4.0
   ...
   contents. They will be called with snek's ``opts`` (:obj:`string.Template` via
...
represents the file content while the second element is a :mod:`snek.operations
...
calling them with :obj:`snek's option dict as argument
<snek.api.create_structure>`. Similarly, :obj:`string.Template.safe_substitute`
are called with snek's opts.
...
For example::
    ...
    from snek.operations import no_overwrite
    struct: Structure = {
        'namespace': {
...
"""See :obj:`snek.actions.ActionParams`"""
...
def define_structure(struct: Structure, opts: ScaffoldOpts) -> ActionParams:
    ...
    Returns:
        Project structure and snek's options
...
def modify(
    ...
    .. versionchanged:: 4.0
       ...
       handling the new :obj:`snek.operations` API.
...
def ensure(
    ...
    Args:
        ...
        file_op: see :obj:`snek.operations`, :obj:`~.create`` by default.

    .. versionchanged:: 4.0
       Instead of a ``update_rule`` flag, the function now accepts a :obj:`file_op
       <snek.oprtations.FileOp>`.
